<template>
	...
	<!-- 	<view class="banner">
		...
		<!-- #endif -->
		<view class="des">
			<text>{{this.detail.price}}</text>
			<text>{{this.detail.title}}</text>
		</view>
		<view class="img-list">
			<text>商品详情</text>
			<image v-for="(item,index) in detail.smallImage" :key="index" :src="item" mode=""></image>
		</view>
	</view>
</template>
...
// 		},
		onLoad(e) {
			this.detail = JSON.parse(decodeURIComponent(e.detailDate));
// 			this.getDetail();
